...
from xlnetmodel import XLNetForTokenClassificationCLS, XLNetForTokenClassificationATT, XLNetForTokenClassificationConcatCLS, XLNetForTokenClassificationONATT
...
def get_special_tokens(tokenizer, tag2idx):
    ...
    pad_tok = tokenizer._convert_token_to_id("<pad>")
    sep_tok = tokenizer._convert_token_to_id("<sep>")
    cls_tok = tokenizer._convert_token_to_id("<cls>")
    o_lab = tag2idx["O"]
...
Classifier = XLNetForTokenClassificationCLS

max_len  = 512
epochs = 8
learning_rate = 5e-6
bert_vocab = "xlnet-large-cased"
batch_num = 4

bert_out_address = './models/xlnet_out_model/xlnet_cls'
...
tags = pad_sequences([[tag2idx.get(l) for l in lab] for lab in word_piece_labels],
                     maxlen=max_len, value=tag2idx['O'], padding="pre",
...
tr_inputs, val_inputs, tr_tags, val_tags,tr_masks, val_masks= train_test_split(input_ids, tags,attention_masks,
                                                            random_state=2021, test_size=0.11)
...
train_dataloader = DataLoader(train_data, sampler=train_sampler, pin_memory=True, batch_size=batch_num,drop_last=True)

valid_data = TensorDataset(val_inputs, val_masks, val_tags)
valid_sampler = SequentialSampler(valid_data)
valid_dataloader = DataLoader(valid_data, sampler=valid_sampler, pin_memory=True, batch_size=batch_num,drop_last=True)
...
for _ in trange(epochs, desc="Epoch"):
    ...
    for step, batch in zip(tqdm(range(len(train_dataloader)),position=0,desc="Data"), train_dataloader):
        ...
        tr_batch_preds = torch.argmax(tr_logits[preds_mask.squeeze()], axis=1)
    ...
    for step, batch in enumerate(valid_dataloader):
        ...
        val_batch_preds = torch.argmax(logits[preds_mask.squeeze()], axis=1)
