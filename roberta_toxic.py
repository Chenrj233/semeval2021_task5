...
from robertamodel import RobertaForTokenClassificationCLS, RobertaForTokenClassificationATT, RobertaForTokenClassificationConcatCLS,RobertaForTokenClassificationONATT
...
def get_special_tokens(tokenizer, tag2idx):
    ...
    sep_tok = tokenizer.convert_tokens_to_ids("</s>")
    cls_tok = tokenizer.convert_tokens_to_ids("<s>")
    o_lab = tag2idx["O"]
...
Classifier = RobertaForTokenClassificationCLS
max_len  = 512
epochs = 8
learning_rate = 5e-6
bert_vocab = 'roberta-large'
batch_num = 4
bert_out_address = './models/roberta_out_model/roberta_cls'
...
tr_inputs, val_inputs, tr_tags, val_tags,tr_masks, val_masks = train_test_split(input_ids, tags,attention_masks,
                                                            random_state=1996, test_size=0.11)
...
train_dataloader = DataLoader(train_data, pin_memory=True, sampler=train_sampler, batch_size=batch_num, drop_last=True)

valid_data = TensorDataset(val_inputs, val_masks, val_tags)
valid_sampler = SequentialSampler(valid_data)
valid_dataloader = DataLoader(valid_data, sampler=valid_sampler, batch_size=batch_num, pin_memory=True, drop_last=True)
