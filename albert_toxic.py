...
from albertmodel import AlbertForTokenClassificationCLS,AlbertForTokenClassificationATT, AlbertForTokenClassificationConcatCLS, AlbertForTokenClassificationONATT
...
def get_special_tokens(tokenizer, tag2idx):
    ...
    cls_tok = tokenizer.convert_tokens_to_ids("[CLS]")
    o_lab = tag2idx["O"]
...
Classifier = AlbertForTokenClassificationCLS

max_len = 512
epochs = 8
learning_rate = 5e-6
bert_vocab = "albert-xlarge-v2"
batch_num = 4
#                                         modelname_malen_epoch_batchnum_lr
bert_out_address = './models/albert_out_model/albert_cls'
...
tokenizer = AlbertTokenizer.from_pretrained(bert_vocab)
...
tr_inputs, val_inputs, tr_tags, val_tags, tr_masks, val_masks = train_test_split(input_ids, tags, attention_masks,
                                                                                 random_state=1228, test_size=0.11)
